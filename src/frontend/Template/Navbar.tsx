...
export default function Navbar(){
    ...
    return(
        <section className="flex w-screen fixed z-20 top-0 bg-card text-card-foreground drop-shadow-sm justify-between py-2 md:p-4">
            <motion.div 
            ...
            >
                <FontAwesomeIcon icon={Pages.icon} className={`${location.pathname === '/ds' ? 'text-foreground' : 'text-blue-400'} transition-all duration-400 ease-in-out text-lg lg:text-2xl`}/>
                <p className="md:font-bold font-medium max-sm:text-xs">{Pages.title}</p>
            </motion.div>
            <motion.div
                className="flex items-center gap-2 lg:gap-5 max-sm:text-sm font-medium md:font-bold mx-2 md:mx-5"
                initial={{ opacity: 0, x: 0 }}
                animate={{ opacity: 1, x: -10 }}
                transition={{
                    duration: 0.4,
